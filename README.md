...
Beyond what the predecessor sites do it adds **source selection**, **purpose selection**, and an export shaped like what the editors assemble in Mailchimp by hand, in either of two styles: the engineering one, grouped by day with their own field labels; or the DaIS one, centred and chronological with a pill link per event and a dotted rule between them. Each was transcribed from an issue that went out, down to the accent colour and the block rhythm — their Mailchimp template groups the title with its attribution and the speaker with the time and place, so ours does too, and the gaps live on the groups. Three of that edition's eight entries carry no attribution and three carry no speaker; grouping is what keeps an absent field from moving a line rather than just removing one. Both the copy and the download carry their styling inline, because email clients strip `<style>` and honour only `style=""` — and the preview carries the same style as the export, so what is on screen is what leaves.
